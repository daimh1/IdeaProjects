...
public class UserDao {
    ...
    public static boolean UpdateUser(String uid, String username, String nickname, String password, String email, String birthday, String registerdate){
        boolean flag=false;
        QueryRunner queryRunner = new QueryRunner(DBCPUtils.getDataSource());
        try {
            int update = queryRunner.update("update user set username=?,nickname=?,password=?,email=?,birthday=? where uid=?",username,nickname,password,email,birthday,uid);
            if (update==1){
                flag =true;
            }
        } catch (SQLException e) {
