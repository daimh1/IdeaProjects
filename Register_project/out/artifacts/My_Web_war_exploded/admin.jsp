...
<%
    ...
    for(int i=0;i<users.size();i++){
        out.print("<tr>");
        out.print("<td>"+users.get(i).getUsername()+"</td>"+"<td>"+users.get(i).getPassword()+"</td>"+"<td>"+users.get(i).getAge()+"</td>"+"<td><a href='/delete?de="+i+"'>删除</a></td>");
        out.println("</tr>");
    }
...
%>
    <form action="${pageContext.request.contextPath}/userServlet?op=register" method="post">
    <tr> <td><input type="text" name="username"></td> <td><input type="text" name="exampleInputPassword1"></td><td><input type="text" name="age"></td><td><button type="submit" class="btn btn-default">提交</button></td></tr>
    </form>
</table>
<a href="${pageContext.request.contextPath}/userServlet?op=logout">注销</a>
</body>
</html>
