...
public class OrderDao {
    ...
    public static boolean UpdateOrder(String oid, Order obj){
        boolean flag=false;
        QueryRunner queryRunner = new QueryRunner(DBCPUtils.getDataSource());
        try {
            int update = queryRunner.update("update orderlist set money=?,recipients=?,tel=?,address=?,state=?,ordertime=?,uid=? where oid=?",obj.getMoney(),obj.getRecipients(),obj.getTel(),obj.getAddress(),obj.getState(),obj.getOrdertime(),obj.getUid(),oid);
            if (update==1){
                flag =true;
            }
        } catch (SQLException e) {
    ...
    public static List<Order> accountOrder(String uid){
        QueryRunner queryRunner = new QueryRunner(DBCPUtils.getDataSource());
        List<Order> query=null;
        try {
            query=queryRunner.query("select * from orderlist where uid=?",uid, new BeanListHandler<Order>(Order.class));
        } catch (SQLException e) {
            e.printStackTrace();
        }
        return query;
