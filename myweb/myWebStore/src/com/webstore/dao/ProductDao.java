...
public class ProductDao {
    ...
    public static boolean UpdateProduct(Product newproduct,String pid){
        ...
        QueryRunner queryRunner = new QueryRunner(DBCPUtils.getDataSource());
        try {
            int update = queryRunner.update("update product set pname=?,estoreprice=?,markprice=?,pnum=?,cid=?,descibe=? where pid=?",newproduct.getPname(),newproduct.getEstoreprice(),newproduct.getMarkprice(),newproduct.getPnum(),newproduct.getCid(),newproduct.getdescibe(),pid);
            if (update==1){
                flag =true;
            }
        } catch (SQLException e) {
    ...
    public static List<Product> namequery(String pname){
        QueryRunner qr = new QueryRunner(DBCPUtils.getDataSource());
        List<Product> products=null;
        try {
            products=qr.query("select * from product where pname like '%"+pname+"%'",new BeanListHandler<Product>(Product.class));
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public static List<Product> multiquery(String pid,String pname,String cid,String price1,String price2) throws SQLException {
        ...
        public static String findpidbyimgurl(String imgurl){
            QueryRunner queryRunner = new QueryRunner(DBCPUtils.getDataSource());
            String pid=null;
            try {
                Product product=queryRunner.query("select * from product where imgurl=?",imgurl,new BeanHandler<Product>(Product.class));
                pid=product.getPid();
            } catch (SQLException e) {
                e.printStackTrace();
            }
        ...
        public static boolean reducepnumByPid(String num,String pid){
            boolean flag=false;
            QueryRunner queryRunner = new QueryRunner(DBCPUtils.getDataSource());

            try {
                int update = queryRunner.update("update product set pnum=pnum-? where pid=?",num, pid);
                if (update==1){
                    flag =true;
                }
            } catch (SQLException e) {
    ...
    public static boolean pluspnumByPid(String num,String pid){
        boolean flag=false;
        QueryRunner queryRunner = new QueryRunner(DBCPUtils.getDataSource());
