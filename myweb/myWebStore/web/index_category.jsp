...
<c:if test="${empty categories }">
	<%
		...
		List<Product> productList= (List<Product>) request.getAttribute("categoryProductList");
	%>
</c:if>
    ...
    <div id="templatemo_main">
		...
		<div id="content" class="float_r">
			<span><h3 style="width:600px;heigth:40px;background: #EEEEEE; padding: 10px"> Products </h3></span>
				...
				<c:forEach items="${categoryProductList }" var="product" varStatus="vs">
			...
			<%--<c:forEach items="${categoryProductList }" var="product" varStatus="vs">--%>
        		<%--<div class="${vs.index % 3 != 2?'product_box':'product_box no_margin_right' }">--%>
		            <%--<h3>${product.pname }</h3>--%>
	            	<%--<a href="${pageContext.request.contextPath }/frontProduct?op=findProductById&pid=${product.pid}">--%>
	            		<%--<img src="file/${product.imgurl }" width="200" height="150" alt="" title="${product.pname }"/>--%>
	            	<%--</a>--%>
	                <%--<p style="width: 200px;overflow: hidden;white-space: nowrap; text-overflow: ellipsis;">${product.descibe}</p>--%>
